...
public class WeizhangActivity extends Activity {
	...
	private final String defaultChepai = "皖"; // 皖=安徽
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		weizhangIntent.putExtra("appId", 1534);// 您的appId
		weizhangIntent.putExtra("appKey", "e848cb8128262b9943172ce494d69763");// 您的appKey
		...
		btn_cpsz = findViewById(R.id.btn_cpsz);
		...
		// 显示隐藏行驶证图示
		popXSZ = findViewById(R.id.popXSZ);
	...
	private void setQueryItem(int cityId) {
		...
		if (cityConfig != null) {
			...
			View row_chejia = findViewById(R.id.row_chejia);
			View row_engine = findViewById(R.id.row_engine);
	...
	private boolean checkQueryItem(CarInfo car) {
		if (car.getCity_id() == 0) {
			Toast.makeText(WeizhangActivity.this, "请选择查询地", 0).show();
			return false;
		}

		if (car.getChepai_no().length() != 7) {
			Toast.makeText(WeizhangActivity.this, "您输入的车牌号有误", 0).show();
			return false;
		}
		...
		if (car.getCity_id() > 0) {
			...
			if (classno > 0) {
				if (car.getChejia_no().equals("")) {
					Toast.makeText(WeizhangActivity.this, "输入车架号不为空", 0).show();
					return false;
				}

				if (car.getChejia_no().length() != classno) {
					Toast.makeText(WeizhangActivity.this, "输入车架号后" + classno + "位", 0).show();
					return false;
				}
			} else if (classno < 0) {
				if (car.getChejia_no().length() == 0) {
					Toast.makeText(WeizhangActivity.this, "输入全部车架号", 0).show();
					return false;
				}
			}
			...
			if (engineno > 0) {
				if (car.getEngine_no().equals("")) {
					Toast.makeText(WeizhangActivity.this, "输入发动机号不为空", 0).show();
					return false;
				}

				if (car.getEngine_no().length() != engineno) {
					Toast.makeText(WeizhangActivity.this, "输入发动机号后" + engineno + "位", 0).show();
					return false;
				}
			} else if (engineno < 0) {
				if (car.getEngine_no().length() == 0) {
					Toast.makeText(WeizhangActivity.this, "输入全部发动机号", 0).show();
					return false;
				}
			}
			...
			if (registno > 0) {
				if (car.getRegister_no().equals("")) {
					Toast.makeText(WeizhangActivity.this, "输入证书编号不为空", 0).show();
					return false;
				}

				if (car.getRegister_no().length() != registno) {
					Toast.makeText(WeizhangActivity.this, "输入证书编号后" + registno + "位", 0).show();
					return false;
				}
			} else if (registno < 0) {
				if (car.getRegister_no().length() == 0) {
					Toast.makeText(WeizhangActivity.this, "输入全部证书编号", 0).show();
					return false;
				}
			}
			return true;
	...
	private void hideShowXSZ() {
		View btn_help1 = findViewById(R.id.ico_chejia);
		View btn_help2 = findViewById(R.id.ico_engine);
