...
public class SwipeRefreshLayout extends ViewGroup {
	...
	private final SwipeProgressBar mProgressBar; // the thing that shows
													// progress is going
	private final SwipeProgressBar mProgressBarBottom;
	...
	// 设置下方进度条的完成度百分比
	private final Animation mShrinkTriggerBottom = new Animation() {
		@Override
		public void applyTransformation(float interpolatedTime, Transformation t) {
			float percent = mFromPercentage + ((0 - mFromPercentage) * interpolatedTime);
	...
	private void setTriggerPercentage(float percent) {
		...
		if (((mMode == Mode.PULL_FROM_START) || (mMode == Mode.BOTH)) && mLastDirection != Mode.PULL_FROM_END
				&& !mLoading) {
			mProgressBar.setTriggerPercentage(percent);
		} else if (((mMode == Mode.PULL_FROM_END) || (mMode == Mode.BOTH)) && mLastDirection != Mode.PULL_FROM_START
				&& !mRefreshing) {
			mProgressBarBottom.setTriggerPercentage(percent);
		}
	}
	...
	private void setColorSchemeResourcesBottom(int colorRes1, int colorRes2, int colorRes3, int colorRes4) {
		final Resources res = getResources();
		setColorSchemeColorsBottom(res.getColor(colorRes1), res.getColor(colorRes2), res.getColor(colorRes3),
				res.getColor(colorRes4));
	}
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (mIsBeingDragged) {
				...
				if (yDiff > mDistanceToTriggerSync) {
					...
					// 根据手指移动距离设置进度条显示的百分比
					setTriggerPercentage(mAccelerateInterpolator.getInterpolation(Math.abs(yDiff)
